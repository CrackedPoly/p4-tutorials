...
def readTableRules(p4info_helper, sw):
    ...
    for response in sw.ReadTableEntries():
        for entity in response.entities:
            ...
            print(p4info_helper.get_name("tables", entry.table_id))
            print(entry)
            print('-----')
...
if __name__ == '__main__':
    ...
    # p4info file contains tables, actions and counters information, defines the specific names of them in runtime.
    if not os.path.exists(args.p4info):
        parser.print_help()
        print("\np4info file not found: %s\nHave you run 'make'?" % args.p4info)
        parser.exit(1)
    # bmv2_json is the abstract syntax tree file generated by p4c
